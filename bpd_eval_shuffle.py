...
p_randomize_tiles_shuffle_blocks_batch = jax.vmap(randomize_tiles_shuffle_blocks_batch, in_axes=(0, None, None, 0))

def tiles_shuffle_blocks(rng, data, M):
    '''
    Args:
      prng: An array of random states. The list dimension equals the number of devices.
    '''
    rng, *perm_rng = jax.random.split(rng, data.shape[0]*data.shape[1] + 1)
    perm_rng = jnp.asarray(perm_rng).reshape((data.shape[0], data.shape[1]) + rng.shape)
    return p_randomize_tiles_shuffle_blocks_batch(data, M, M, perm_rng)
    ...
    for repeat in range(begin_repeat_id, bpd_num_repeats):
      ...
      for batch_id in range(begin_batch_id, len(ds_bpd)):
        ...
        #   step_rng = jnp.asarray(step_rng)
          batch['image'] = tiles_shuffle_blocks(step_rng, batch['image'], block_shuffle)
        #   batch['image'] = p_randomize_tiles_shuffle_blocks_batch(batch['image'], block_shuffle, block_shuffle, jnp.array(perm_rng))
